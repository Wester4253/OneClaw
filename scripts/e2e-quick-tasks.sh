...
DEBUG_ACTION="${DEBUG_ACTION:-xyz.westr42.oneclaw.DEBUG_TASK}"
...
RESULTS_FILE="${RESULTS_FILE:-/tmp/oneclaw-e2e-${MODE}-quick-tasks-$(date +%Y%m%d-%H%M%S).log}"
...
resolve_local_model() {
    ...
    local models_dir="/storage/emulated/0/Android/data/${APP_ID}/files/models"
...
configure_mode() {
    case "$MODE" in
        cloud)
            if [ -z "${OPENAI_API_KEY:-}" ] && [ -f /home/nicole/MyGithub/OneClaw/.env ]; then
                # shellcheck disable=SC1091
                source /home/nicole/MyGithub/OneClaw/.env
            fi
            if [ -z "${OPENAI_API_KEY:-}" ]; then
                echo "OPENAI_API_KEY not set and .env not available"
                exit 1
            fi
            adb_retry adb shell "am broadcast -a $DEBUG_ACTION -p $APP_ID --es task 'config:' --es api_key '$OPENAI_API_KEY' --es model_name '$CLOUD_MODEL_NAME'" >/dev/null
            ;;
        local)
            resolve_local_model
            adb_retry adb shell "am broadcast -a $DEBUG_ACTION -p $APP_ID --es task 'config:' --es provider 'LOCAL' --es base_url '$LOCAL_MODEL_PATH' --es model_name '$LOCAL_MODEL_NAME'" >/dev/null
            ;;
        -h|--help|help)
            print_usage
            exit 0
...
cancel_running_task() {
    adb_retry adb shell "am broadcast -a $DEBUG_ACTION -p $APP_ID --es task 'cancel:'" >/dev/null 2>&1 || true
...
dismiss_blocking_system_dialogs() {
    ...
    if printf '%s' "$window_state" | grep -q "Application Not Responding: $APP_ID"; then
...
wait_for_accessibility() {
    ...
    for attempt in $(seq 1 "${ACCESSIBILITY_WAIT_ATTEMPTS:-10}"); do
        if adb shell dumpsys accessibility 2>/dev/null | grep -q "Bound services:{Service\\[label=OneClaw"; then
            return 0
        fi
        sleep 1
    done
    echo "    WARN — OneClaw Accessibility service did not report bound before task start" | tee -a "$RESULTS_FILE"
    return 1
}
...
reset_foreground() {
    ...
    dismiss_blocking_system_dialogs
    adb_retry adb shell am start -n "$APP_ID/$MAIN_ACTIVITY" >/dev/null 2>&1 || true
...
run() {
    ...
    sleep 1
    adb_retry adb shell "am broadcast -a $DEBUG_ACTION -p $APP_ID --es task '$task'" >/dev/null 2>&1
...
echo "  ONECLAW E2E — QUICK TASKS (${MODE^^})" | tee -a "$RESULTS_FILE"
...
if [ "$MODE" = "cloud" ]; then
    run "Reddit OneClaw"        "Open Reddit and search for OneClaw" 60
